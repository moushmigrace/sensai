...
// ── Hub registry (backend `hubs` table) ─────────────────────────────────────

export async function listCourseHubs(
    courseId: number,
): Promise<HubSummary[]> {
    const res = await fetch(`${BASE}/hub/courses/${courseId}/hubs`, {
        cache: "no-store",
    });
    if (!res.ok) return [];
    return res.json();
...
export async function getHubById(hubId: number): Promise<HubDetail | null> {
    const res = await fetch(`${BASE}/hub/hubs/${hubId}`, { cache: "no-store" });
